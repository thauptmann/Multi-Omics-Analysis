...
class DenseNetGenerator(NetworkGenerator):
    ...
    def generate(self, model_len=None, model_width=None):
        ...
        # First dense
        output_node_id = 0
        output_node_id = graph.add_layer(StubDense(temp_input_channel, model_width),
                                         output_node_id)
        output_node_id = graph.add_layer(self.batch_norm(num_features=model_width), output_node_id)
        db_input_node_id = graph.add_layer(StubReLU(), output_node_id)
        # Each DenseBlock
        ...
        # Linear layer
        graph.add_layer(StubDense(num_features, self.n_output_node), db_input_node_id)
        return graph

    def _dense_block(self, num_layers, num_input_features, growth_rate, drop_rate, graph, input_node_id):
        block_input_node = input_node_id
        for i in range(num_layers):
            input_size = num_input_features + i * growth_rate
            output = num_input_features + (i + 1) * growth_rate
            block_input_node = self._dense_layer(input_size, output, drop_rate, graph, block_input_node)
        return block_input_node

    def _dense_layer(self, num_input_features, output_size, drop_rate, graph, input_node_id):
    ...
    def _transition(self, num_input_features, num_output_features, graph, input_node_id, dropout_rate):
        out = graph.add_layer(StubDense(num_input_features, num_output_features), input_node_id)
        out = graph.add_layer(self.batch_norm(num_features=num_output_features), out)
        out = graph.add_layer(StubReLU(), out)

    ...
    for train_index_outer, test_index in tqdm(skf_outer.split(GDSCE, GDSCR), total=skf_outer.get_n_splits(),
        ...
        for hyperparameters_set in hyperparameters_set_list:
            ...
            for train_index, validate_index in tqdm(skf.split(X_train_valE, Y_train_val), total=skf.get_n_splits(),
                ...
                # latent feature integration
                integration_Supervised_Encoder = SupervisedEncoder(OE_dim + OM_dim + OC_dim, integration_width, E_dr)
                integration_Supervised_Encoder.to(device)
                ...
                for m_epoch in range(M_Supervised_Encoder_epoch):
                    ...
                    for i, (_, dataM, _, target) in enumerate(trainLoader):
                        if torch.mean(target) != 0. and torch.mean(target) != 1. and len(target) > 2:
                            ...
                            encoded_Triplets_list = TripSel(encoded_M, target)
                            integrated_loss = trip_loss_fun(encoded_M[encoded_Triplets_list[:, 0], :],
                                                            encoded_M[encoded_Triplets_list[:, 1], :],
                                                            encoded_M[encoded_Triplets_list[:, 2], :])
                    ...
                    with torch.no_grad():
                        ...
                        encoded_Triplets_list = TripSel(encoded_val_M, torch.FloatTensor(Y_val))
                        val_integrated_loss = trip_loss_fun(encoded_val_M[encoded_Triplets_list[:, 0], :],
                                                            encoded_val_M[encoded_Triplets_list[:, 1], :],
                                                            encoded_val_M[encoded_Triplets_list[:, 2], :])

                        if pre_loss <= val_integrated_loss:
                            break_num += 1

                        if break_num > 1:
                            pass  # break
                        else:
                            pre_loss = val_integrated_loss
                ...
                # train integration latent
                pre_loss = 100
                break_num = 0
                for m_epoch in range(integration_epoch):
                    integration_Supervised_Encoder.train().to(device)
                    flag = 0
                    for i, (dataE, dataM, dataC, target) in enumerate(trainLoader):
                        if torch.mean(target) != 0. and torch.mean(target) != 1. and len(target) > 2:
                            ...
                            encoded_Triplets_list = TripSel(concat_encoded, target)
                            integrated_loss = trip_loss_fun(concat_encoded[encoded_Triplets_list[:, 0], :],
                    ...
                    with torch.no_grad():
                        ...
                        val_integrated_loss = trip_loss_fun(concat_encoded[encoded_Triplets_list[:, 0], :],
                                                            concat_encoded[encoded_Triplets_list[:, 1], :],
                                                            concat_encoded[encoded_Triplets_list[:, 2], :])
        ...
        # integration encoder
        for m_epoch in range(integration_epoch):
            integration_Supervised_Encoder.train()
            for i, (dataE, dataM, dataC, target) in enumerate(trainLoader):
                if torch.mean(target) != 0. and torch.mean(target) != 1. and len(target) > 2:
                    dataE = dataE.to(device)
                    dataM = dataM.to(device)

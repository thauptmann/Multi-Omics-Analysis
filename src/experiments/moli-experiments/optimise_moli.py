...
def moli(search_iterations, experiment_name, drug_name, extern_dataset_name, gpu_number):
    device, pin_memory = create_device(gpu_number)

    result_path = Path('..', '..', '..', 'results', 'moli', drug_name, experiment_name)
...
if __name__ == '__main__':
    ...
    if args.drug == 'all':
        for drug, extern_dataset in parameter['drugs'].items():
            moli(args.search_iterations, args.experiment_name, drug, extern_dataset, args.gpu_number)
    else:
        extern_dataset = parameter['drugs'][args.drug]
        moli(args.search_iterations, args.experiment_name, args.drug, extern_dataset, args.gpu_number)

...
class StackingSigmoidModel(nn.Module):
    def __init__(self, input_sizes, encoding_sizes, dropout, stacking_type, use_reconstruction):
        ...
        self.e_classify = nn.Sequential(nn.Linear(encoding_sizes[0], 1), nn.Sigmoid())
        self.m_classify = nn.Sequential(nn.Linear(encoding_sizes[1], 1), nn.Sigmoid())
        self.c_classify = nn.Sequential(nn.Linear(encoding_sizes[2], 1), nn.Sigmoid())
    ...
    def forward(self, expression, mutation, cna):
        ...
        if self.stacking_type == 'less_stacking':
            ...
            classification = self.classify_all(torch.concat((classified_e, classified_m, classified_c, classified_emc),
                                                            dim=1))
            if self.use_reconstruction:
                return [classification, torch.concat((encoded_e, encoded_m, encoded_c), dim=1), expression_reconstruction,
                        mutation_reconstruction, cna_reconstruction]
            else:

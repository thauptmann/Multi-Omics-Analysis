        ...
        if self.stacking_type == 'less_stacking':
            ...
            if self.stacking_type == 'all':
                ...
                classification = self.classify_all(
                    torch.concat((classified_e, classified_m, classified_c, classified_em,
                                  classified_mc,
...
class StackingSplittedModel(nn.Module):
    def __init__(self, encoding_sizes, dropout, stacking_type):
        super(StackingSplittedModel, self).__init__()
        self.stacking_type = stacking_type
        ...
        self.e_classify = nn.Sequential(nn.Dropout(dropout), nn.Linear(e_encoding_size, 1), nn.Sigmoid())
        self.m_classify = nn.Sequential(nn.Dropout(dropout), nn.Linear(m_encoding_size, 1), nn.Sigmoid())
        self.c_classify = nn.Sequential(nn.Dropout(dropout), nn.Linear(c_encoding_size, 1), nn.Sigmoid())
        self.em_classify = nn.Sequential(nn.Dropout(dropout), nn.Linear(em_encoding_size, 1), nn.Sigmoid())
        self.mc_classify = nn.Sequential(nn.Dropout(dropout), nn.Linear(mc_encoding_size, 1), nn.Sigmoid())
        self.ec_classify = nn.Sequential(nn.Dropout(dropout), nn.Linear(ec_encoding_size, 1), nn.Sigmoid())
        self.emc_classify = nn.Sequential(nn.Dropout(dropout), nn.Linear(emc_encoding_size, 1), nn.Sigmoid())
        self.classify_all = nn.Linear(stacking_dimension, 1)
